# 開発哲学・実装ガイドライン (Wanyaldee) — AGENTS.md 断片

> `dev-philosophy` Claude Code スキルの単体 `AGENTS.md` 版。オンデマンドで
> 呼び出す Skill 機構を持たず、`AGENTS.md` を常時受動的に読み込むエージェント
> (Codex CLI, Cursor 等)向け。正本は [SKILL.md](SKILL.md) — 編集はそちらに
> 行い、この断片を追随させる。原文との意図的な差分は「Claude への行動指示」
> という見出しを一般化した1箇所のみ。他プロジェクトでの使い方はリポジトリ
> 直下の [`AGENTS.md`](../../AGENTS.md) または [`README.md`](../../README.md)
> を参照。
