# Injection vigilance — AGENTS.md fragment

> Standalone `AGENTS.md` copy of the `injection-vigilance` Claude Code
> skill, for agents that read `AGENTS.md` passively instead of invoking a
> Skill on demand. [SKILL.md](SKILL.md) is authoritative — edit it first,
> then bring this fragment in line. The only intentional difference from
> `SKILL.md` is generalizing the list of "system configuration" file names
> in the rule below beyond Claude Code's own (`CLAUDE.md`). To use this in
> another project, see the root [`AGENTS.md`](../../AGENTS.md) or
> [`README.md`](../../README.md).
